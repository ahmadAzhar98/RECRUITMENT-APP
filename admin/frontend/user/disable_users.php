...
<body>
    ...
    <ul>
        <?php
        if (isset($_SESSION['disable_users']) && is_array($_SESSION['disable_users'])) {
            foreach ($_SESSION['disable_users'] as $user) {
                echo '<li>';
                echo 'Name: ' . htmlspecialchars($user['name']) . '<br>';
                echo 'Email: ' . htmlspecialchars($user['email']) . '<br>';
                echo ' <a href="../../backend/user/reEnable_user.php?usr_id=' . htmlspecialchars($user['user_id']) . '">Enable</a>';
                echo '</li>';
            }
        } else {
            echo '<li>No disabled users available.</li>';
        }
        ?>
    </ul>
